...
export default function UserList() {
	...
	//* Creating User and closing modal
	...
	const saveCreateUserClickHandler = (e) => {
		...
		userService.createUser(formValues).then((data) => setUserList((users) => [...users, data]));
		setShowCreate(false);
	};

	//* User Details: Getting user id and making function to close the modal
	const userDetailsClickHandler = (userId) => {
		setUserInfoId(userId);
	};

	const closeUserDetailsClickHandler = () => {
		setUserInfoId(undefined);
	};

	//* User Delete: Getting user id, handling the deletion of the user and making function to close the modal
	...
	const userDeleteHandler = async () => {
		await userService.deleteUser(userDeleteId);

		setUserList((state) => state.filter((user) => user._id !== userDeleteId));
		setUserDeleteId(null);
	};

	//* User Edit: Getting user id, passing the id to the component and handling the modal close
	const userEditDeleteHandler = (userId) => {
	...
	const saveEditUserClickHandler = async (e) => {
		...
		const updatedUser = await userService.editUser(userEditId, formValues);

		setUserList((state) => state.map((user) => (user._id === userEditId ? updatedUser : user)));
		setUserEditId(null);
	};
	...
	return (
		<section className="card users-container">
			...
			{userInfoId && (
				<UserDetails onClose={closeUserDetailsClickHandler} userId={userInfoId} />
			)}

			{userDeleteId && (
				<UserDelete onClose={userDeleteCloseHandler} onDelete={userDeleteHandler} />
			)}
